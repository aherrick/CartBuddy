...
<img src="CartBuddy.Client/wwwroot/cartbuddy.png" width="400" alt="CartBuddy">
...
- Search products
- Build a cart and start checkout (OAuth)
...
- .NET Minimal APIs (server)
- Shared Kroger data/client layer
- Shared models for client/server DTOs
...
- `CartBuddy.Data` — shared Kroger API client, parsing, and OAuth/cart helpers
...
- `CartBuddy.Shared` — Shared models
	...
	public async Task<KrogerProductSearchPage> SearchProductsAsync(
		string term,
		string locationId,
		int start = 0,
		int limit = 4
	...
	public async Task<List<KrogerLocation>> SearchLocationsAsync(string zipCode, int limit = 10)
	{
		var token = await GetClientTokenAsync();
		var requestUri = new Uri(
			BaseUri,
		...
		var locations = new List<KrogerLocation>();
		if (!document.RootElement.TryGetProperty("data", out var data))
		{
			return locations;
		}
	...
	public async Task AddToCartAsync(string userToken, IEnumerable<KrogerCartItem> items)
	{
		var cartItems = items
